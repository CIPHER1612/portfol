...
**Last Updated**: June 2026 — **Mobile performance pass.** PageSpeed (mobile, Slow 4G) was 42 with a 14.8s LCP. Fixes: the intro headline is now server-rendered full text (was a JS typewriter that left the LCP element empty until hydration), with the typewriter restored as a **desktop-only** enhancement (hidden via `.splash-pretype` until typing starts; mobile shows the text instantly); `NetworkCanvas` is lazy-loaded (`NetworkCanvasLazy`, `ssr:false`), cuts particles on mobile, honours `prefers-reduced-motion`, and pauses via `IntersectionObserver` + `visibilitychange` when offscreen/hidden; removed the unused `gsap` dependency. **Result:** TBT 1240→560ms, main-thread 8.6→4.7s, JS exec 3.4→1.8s, CLS 0.032→0.018 (all improved); the headline LCP element paints in ~1.3–4.0s in controlled throttled runs (verified via Playwright) — PageSpeed's reported 14.2s is dominated by lab variance + its harsh CPU throttle, not a real defect. Score nudged 42→48. Field data (Search Console CrUX) is the real signal to watch.  
...
**Mobile Performance**: ✅ (LCP fix + canvas guards shipped & measured — TBT/main-thread roughly halved; LCP element is the SSR headline, ~1.3–4.0s controlled. Lab score 48 is variance-dominated; accepted as good-enough for an animation-rich portfolio. **Deferred lever:** code-split below-fold sections / trim framer-motion — medium effort, uncertain lab payoff, has SSR/SEO + snap-scroll trade-offs)  
**Next Task** (content & distribution — the build is feature-complete): **(1)** replace `public/logo.jpeg` with a proper 1200×630 branded OG banner; **(2)** add real project case studies in Sanity + delete the fabricated `portfolioTestimonial` docs (and optionally remove the unused `fetchTestimonials`/`Testimonial` type/schema); **(3)** distribution — watch Search Console Core Web Vitals, pursue backlinks (Clutch.co, Awwwards), consider a blog for long-tail SEO. (Deferred/low-priority: promote CSP-Report-Only to enforced; gitignore `tsconfig.tsbuildinfo`; add `!.env.example` exception)
